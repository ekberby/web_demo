...
class UserDetailServiceTest extends Specification{
    ...
    def "test loadUserByUsername"(){
        def id = "a"
        UserDetails userDetails = new UserPrincipal(User.builder().username("a").password("a").firstName("a")
                .lastName("a").email("a").role(Role.LANDLORD).build())
        def optional = Optional.of(User.builder().username("a").password("a").firstName("a")
                .lastName("a").email("a").role(Role.LANDLORD).build())
        when:
